use std::{fmt::Debug, sync::{atomic::{AtomicBool, Ordering}, Arc, Mutex}};

use winit_core::{event::{ElementState, Ime, KeyEvent, WindowEvent}, keyboard::Key};
use winit_core::keyboard::{PhysicalKey, NamedKey, KeyCode, NativeKeyCode, KeyLocation};

use super::{event_loop::sink::EventSink, WindowId};
                        ...
                        match event {
                            maliit::events::InputMethodEvent::Text(txt) => {
                                if txt.chars().count() == 1 {
                                    ...
                                    let preedit_event = WindowEvent::Ime(Ime::Preedit((&txt).into(), Some((1, 1))));
                                    let commit_event = WindowEvent::Ime(Ime::Commit(txt.into()));
...
fn kb_input_event_from_str(input: &str) -> WindowEvent {
    let key_event = KeyEvent {
        ...
        logical_key: Key::Character(input.into()),
        text: Some(input.into()),
        repeat: false,
        state: ElementState::Pressed,
        location: KeyLocation::Standard,
        text_with_all_modifiers: None,
        key_without_modifiers: Key::Dead(None),
    };
    WindowEvent::KeyboardInput { device_id: None, event: key_event, is_synthetic: false }
}

fn kb_input_event_from_key(key: maliit::events::Key, pressed: bool) -> WindowEvent {
    let key_event = KeyEvent {
        physical_key:  PhysicalKey::Code(maliit_key_to_key_code(key)),
        logical_key: Key::Named(maliit_key_to_named_key(key)),
        text: None,
        repeat: false,
...
fn maliit_key_to_named_key(key: maliit::events::Key) -> NamedKey {
    match key {
        maliit::events::Key::Enter => NamedKey::Enter,
        maliit::events::Key::Backspace => NamedKey::Backspace
    }
}

fn maliit_key_to_key_code(key: maliit::events::Key) -> KeyCode {
    match key {
        maliit::events::Key::Enter => KeyCode::Enter,
        maliit::events::Key::Backspace => KeyCode::Backspace
    }
}


impl Debug for MaliitInputMethod {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "MaliitInputMethod {{}}")
    }
}
