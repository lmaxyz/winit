use crate::{
    ...
    platform_impl::wayland::{logical_to_physical_rounded, window::WindowState},
};
use std::sync::{
    ...
    pub fn show(&self) {
        if !self.is_events_handling_enabled.load(Ordering::Relaxed) {
            ...
            self.events_sink
                .lock()
                .unwrap()
                .push_window_event(WindowEvent::RedrawRequested, self.window_id);
        }
        println!("Showed");
    }
    ...
    pub fn hide(&self) {
        if self.is_events_handling_enabled.fetch_and(false, Ordering::Relaxed) {
            if let Ok(mut window_state) = self.window_state.lock() {
                ...
                let resize_event = WindowEvent::Resized(logical_to_physical_rounded(
                    new_size,
        ...
        std::thread::spawn(move || loop {
                    ...
                    match event {
                        maliit::events::InputMethodEvent::Text(txt) => {
                            if txt.chars().count() == 1 {
                                events_sink
                                    .push_window_event(kb_input_event_from_str(&txt), window_id);
                            } else {
                                let preedit_event = WindowEvent::Ime(crate::event::Ime::Preedit(
                                    (&txt).into(),
                                    Some((0, txt.len())),
                                ));
                                let commit_event =
                                    WindowEvent::Ime(crate::event::Ime::Commit(txt.into()));
                        ...
                        maliit::events::InputMethodEvent::Key { key, pressed } => {
                            events_sink.push_window_event(
                                kb_input_event_from_key(key, pressed),
                                window_id,
                            );
                        },
            ...
            if !is_events_handling_enabled.load(Ordering::Relaxed) {
                events_sink
                    .lock()
                    .unwrap()
                    .push_window_event(WindowEvent::Ime(crate::event::Ime::Disabled), window_id);
                break;
            }
        });
    }
...
fn kb_input_event_from_str(input: &str) -> WindowEvent {
    ...
    let key_event = KeyEvent {
        ...
        platform_specific: crate::platform_impl::KeyEventExtra {
            text_with_all_modifiers: None,
            key_without_modifiers: crate::keyboard::Key::Dead(None),
        },
    };
    WindowEvent::KeyboardInput { device_id, event: key_event, is_synthetic: false }
}
