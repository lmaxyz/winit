...
impl WindowState {
    ...
    pub fn new(
        ...
        extended_surface.as_ref().map(|es| es.update_generic_property("STATUSBAR_VISIBLE".to_string(), vec![1, 0, 0, 0, 1, 0]));
        extended_surface.as_ref().map(|es| es.update_generic_property("BACKGROUND_VISIBLE".to_string(), vec![1, 0, 0, 0, 1, 0]));
        ...
        if let Some(extended_surface) = self.extended_surface.as_ref() {
            ...
            match transform {
                Transform::Normal | Transform::Flipped180 => {
                    ...
                    println!("{:?}", qvariant_true);
                    extended_surface.update_generic_property("STATUSBAR_VISIBLE".to_string(), vec![1, 0, 0, 0, 1, 0]);
                },
                Transform::_180 | Transform::_270 => {
                    let mut qvariant_true = QVariant::from_bool(false).as_bytes().to_vec();
                    qvariant_true.push(0);
                    println!("{:?}", qvariant_true);
                    extended_surface.update_generic_property("STATUSBAR_VISIBLE".to_string(), vec![1, 0, 0, 0, 0, 0]);
                },
                _ => {
                    let mut qvariant_true = QVariant::from_bool(true).as_bytes().to_vec();
                    qvariant_true.push(0);
